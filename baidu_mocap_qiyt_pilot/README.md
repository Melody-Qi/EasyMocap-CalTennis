...
| `make_three_camera_video_montage.py` | 我 | 将三路单目网格结果并排为同步检查视频。 |
...
### 多球片段补充测试（545–565 s，600 帧）

- 作业 `997138_1~3` 三路均正常完成；三路运动员检出率和关键点可用率均为 100%。
- 平均人体框高分别为 160.9、121.7、249.5 px；camera 2 最远，仍能持续检出，但精细手腕姿态风险最高。
- 裁剪后的单目网格总体跟随人体；该时间窗仍以走动、准备和一次上举动作为主，不能单凭它证明高速挥拍精度。
- 人体速度互相关没有形成可靠同步峰值，因此不据此估计三路帧偏移。
- 量化结果保存于 `work/pilot_eval_duoqiu_545_565s.json`；视频和模型输出不进入 Git。
